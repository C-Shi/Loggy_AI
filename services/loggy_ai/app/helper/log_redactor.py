...
class LogRedactor:
    ...
    def __init__(self) -> None:
        ...
        self.jwt_pattern = re.compile(
            r"\beyJhbGciOi[-_a-zA-Z0-9\.]+\.[-_a-zA-Z0-9\.]+\.[-_a-zA-Z0-9\._~]*\b"
        )

        # 2. Generic API Key / Secret Detection (Matches hex/base64/bearer variants)
        self.api_key_pattern = re.compile(
            r'(?i)(api[_-]?key|secret|password|bearer|auth|token)["\s]*[:=]["\s]*'
            r"[a-zA-Z0-9_\-\.\~]{16,64}"
        )
    ...
    def _redact_value(self, value: str) -> str:
        """Apply destructive placeholder masks to a single string value."""
        ...
        value = self.api_key_pattern.sub(
            lambda m: (
                m.group(0).split(":")[0] + ': "[REDACTED_SECRET]"'
                if ":" in m.group(0)
